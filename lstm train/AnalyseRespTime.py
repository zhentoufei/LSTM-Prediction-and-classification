...
def ReadAllFile():
	...
	print 'end read!'
	#ata=np.concatenate((subject108,subject105,subject103,subject106,subject101,subject102,subject104,subject107,subject109))
	data=np.concatenate((subject101,subject102))
	print data.shape
	X_train=data[0:data.shape[0],3:data.shape[1]]
	y_train=data[0:data.shape[0],1]
	X_test=data[0:data.shape[0],3:data.shape[1]]
	y_test=data[0:data.shape[0],1]
...
model_LSTM = model_from_json(open('my_model_architecture.json').read())    
model_LSTM.load_weights('my_model_weights.h5')    
#model = load_model('my_model_weights.h5')  
import time 

for x in range(100,1000,50):
	time1 = time.time()
	score_LSTM = model_LSTM.predict(X_test[1000:1000+x], batch_size=1, verbose=0)
	time2 = time.time()
	print time2-time1
'''print 'Start figure'
# plot results
